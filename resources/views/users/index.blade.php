...
@section('content')
                    ...
                    @if (Auth::user()->user_type_id == 4 || Auth::user()->user_type_id == 5)
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><button type="button" class="btn btn-block btn-primary btn-sm"
                                    data-toggle="modal" data-target="#memberslist"><i class="fa fa-plus"> Add </i></button>
                            </li>
                        </ol>
                    @else
                    @endif
                                ...
                                <tbody>
                                    @foreach ($members as $key => $memberslist)
                                        <tr>
                                            ...
                                            <td>{{ $memberslist->full_name }}</td>
                                            <td>{{ $memberslist->email }}</td>
                                            <td>{{ $memberslist->phone }}</td>
                                            <td>{{ $memberslist->status }}</td>
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu" role="menu">
                                                        <a class="dropdown-item" href="{{ url('/edituser', $memberslist->userID) }}">Edit User</a>

                                                        <a onclick="userdatas('{{ $memberslist->id }}','{{ $memberslist->username }}','{{ $memberslist->pas }}','{{ $memberslist->full_name }}','{{ $memberslist->profile_photo }}','{{ $memberslist->username }}','{{ $memberslist->district_name }}','{{ $memberslist->taluk_name }}','{{ $memberslist->email }}','{{ $memberslist->pas }}','{{ $memberslist->phone }}','{{ $memberslist->status }}')" type="button" class="dropdown-item">View User</a>
    ...
    <div class="modal fade" id="memberslist">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Add Taluk Users Details</h4>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        <label for="" class="col-sm-4 col-form-label"><span style="color:red"></span>Taluk Name
                        </label>
                        <label for="" class="col-sm-8 col-form-label"><span style="color:red"
                    ...
                    <center>
                        <a class="btn btn-info" id="msgbtn"
                            href=""
...
@push('page_scripts')
    <script>
        function userdatas(id,username,pas, full_name, profile_photo, username, district_name, taluk_name, email, pas, phone, status) {
            $("#id").text(id);
